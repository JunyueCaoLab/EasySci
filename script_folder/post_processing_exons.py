...
def merge_gene_count_files(input_folder, output_folder, sampleID, RT_matching_file, pipeline):
    ...
    for i, sample in enumerate(sample_list):
        ...
        sparse_matrix = sp.coo_matrix((count_matrix[:,2], ((count_matrix[:,0] - 1 ), (count_matrix[:,1] - 1))), shape=(gene_annotation.shape[0], cell_annotation.shape[0]), dtype=float)
    ...
    final_gene_annotation = gene_annotation.iloc[:,[0,1,2]]
    final_gene_annotation.columns = ['exon_id', 'gene_type', 'gene_name']
