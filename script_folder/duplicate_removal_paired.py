...
def rm_dup_samfile(input_folder, output_folder, sampleID):
    ...
    while(line1):
        ...
        if (line1[0] == '@'):
            ...
            barcode_UMI1 = name1[0] + name1[1]
            chrom_num1 = (line1.split('\t'))[2]
            start_site1 = (line1.split('\t'))[3]
            
            line2 = f1.readline()
            chrom_num2 = (line2.split('\t'))[2]
            start_site2 = (line2.split('\t'))[3]
            current_location = chrom_num1 + "-" + start_site1 + "-" + chrom_num2 + "-" + start_site2

            if((line1.split('\t'))[0] != (line2.split('\t'))[0]):
                print("WARNING: not proper read pairing during duplicate removal")

            if ((barcode_UMI1 == pre_barcode_UMI)):    
                dup = False
